...
//페이지 블록 갯수 자동화  완벽히 않될수도 있고. 오류가 날수도 있고 보장은 못하지만.
<?php
include("connect.php");
...
if($page+1>$total_page) {} // 현재 페이지에 1을 더해서 결과값이 총페이지값보다 클경우, 아닐경우 for구문 페이징을 한다
  ...
  //echo "<a href=?page=$i>[$i]</a>"; 수정전 코드
  if($cut_block > $total_block) { echo "<a href=?page=$i>[$i]</a>";} // 기준 블록값보다 적으면 블록 끝
  elseif($cut_block <= $total_page) {echo "<a href=?page=$i>[$i]</a>";} // 기준 블록보다 페이지가 많으면 계속 뛰우게 가능하게.
  else{}
